...
\item read a lattice from common \ele or \madx files\footnote{Supported element types are
    listed in \cref{sec:types}.}
\item import closed orbit, optics calculations and particle trajectories from \ele or
  \madx
\item configure and execute \ele (or \madx) automatically
\item track multiple spins at once using threads

\item consider synchrotron radiation for beam dynamics:
  \begin{itemize}
  \item full 6D phase space via import from \ele (or \madx)
...
\item configure a magnet to apply an an-harmonic time dependent magnetic field
\end{itemize}
