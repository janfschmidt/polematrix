...
\begin{configdocgroup}{rfMagnets}
  ...
  time dependent alternating field (rf field).\todo{formula} The setup only affects the
  spin tracking and not the particle tracking with \ele or \madx. In addition, the field
  has to be set up in \ele/\madx, if the influence of an rf field on beam dynamics should
  be included.
  ...
  \begin{configdoc}{Q1}{string}{}[]
    Frequency of the rf field at the beginning of the spin tracking (turn 1) normalized to
    the revolution frequency (tune). [for multiple elements as comma separated list]
  \end{configdoc}

  \begin{configdoc}{dQ}{string}{}[]
    Frequency change per turn (frequency sweep) -- also as a tune. [for multiple elements
